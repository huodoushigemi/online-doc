...
import { type ColDef, type GridApi, type CellRange } from 'ag-grid-community';
...
import type { DataTableProps, Field, TableRow, TableConfig, ExportConfig } from './types';
export * from './types'
...
const DataTable: Component<DataTableProps> = (props) => {
  ...
  const [selectedRange, setSelectedRange] = createSignal<CellRange[]>([]);
  ...
  const removeField = (fieldId: string) => {
    const arr = fields()
    remove(arr, field => field.id === fieldId)
    setFields(arr);
    props.onFieldsChange?.(arr);
  };
  ...
  const deleteSelectedRows = () => {
    if (selectedRange().length > 0) {
      const selectedIds = selectedRange().map(range => 
        data()[range.startRow!.rowIndex!]?.id
      ).filter(Boolean);
  ...
  return (
    <div class="data-table-container gap-4">
      {/* 工具栏 */}
      <div class="toolbar">
        <div class="toolbar-left">
          <button class="btn btn-primary btn-sm" onClick={addRow}>添加行</button>
          <button class="btn btn-error btn-sm"  onClick={deleteSelectedRows} disabled={selectedRange().length === 0}>删除选中</button>
        </div>

        <div class="toolbar-right">
          <Show when={config().enableCopyPaste}>
            <button class="btn btn-outline btn-sm" onClick={copyToClipboard}>复制</button>
            <button class="btn btn-outline btn-sm" onClick={pasteFromClipboard}>粘贴</button>
          </Show>
          <Show when={config().enableExcelExport}>
            <button class="btn btn-outline btn-sm" onClick={() => exportData('csv')}>导出 CSV</button>
      ...
      {/* 数据表格 */}
      <div class={`ag-grid-container ag-theme-quartz`} style='align-self: start;'>
        <AgGridSolid
          ref={e => gridApi = e.api}
          ...
          onColumnMoved={() => {
            ...
            const reorderedFields = columns.map(col => newFields.find(field => field.id === col.field)).filter(Boolean) as Field[]
            setFields(reorderedFields)
            props.onFieldsChange?.(reorderedFields)
            props.onColumnReorder?.(columns.map(e => e.field!))
          }}
        />
      </div>
