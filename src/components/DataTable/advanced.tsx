...
export function RangeSelectorPlugin(): Plugin {
  ...
  return {
    ...
    processProps: {
      thProps: (_, { store }) => (o) => {
        ...
        return { class: inx ? 'col-range-highlight' : '' }
      },
      td: ({ td }, { store }) => (o) => {
  ...
  return {
    processProps: {
      table: ({ table }, { store }) => component(({ children, ...props }) => {
        ...
        createEventListener(() => el, 'pointerdown', () => el.focus())
